...
let currentLang='it';const translations={};fetch('https://gitechnolo.github.io/biotechproject/translations.json').then(response=>{if(!response.ok)throw new Error('File translations.json non trovato o danneggiato');return response.json()}).then(data=>{Object.assign(translations,data);const savedLang=localStorage.getItem('preferred-language');const userLang=savedLang||(navigator.language.startsWith('en')?'en':'it');setLanguage(userLang)}).catch(err=>{console.warn('Traduzioni non disponibili:',err)});function setLanguage(lang){if(!translations[lang])return;document.querySelectorAll('[data-lang-key]').forEach(el=>{const key=el.getAttribute('data-lang-key');if(translations[lang]&&translations[lang][key]){el.textContent=translations[lang][key]}});const flag=document.getElementById('lang-flag');const text=document.getElementById('lang-text');const button=document.getElementById('lang-toggle');const label=document.getElementById('lang-label');if(lang==='it'){flag.textContent='🇮🇹';text.textContent='Italiano';label.textContent='Cambia lingua';button.setAttribute('aria-label','Cambia lingua in inglese')}else{flag.textContent='🇬🇧';text.textContent='English';label.textContent='Switch language';button.setAttribute('aria-label','Switch to Italian')}
localStorage.setItem('preferred-language',lang);currentLang=lang}
