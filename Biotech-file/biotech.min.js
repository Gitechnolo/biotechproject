...
function applyTranslations(translations,lang){document.querySelectorAll('[data-lang-key]').forEach(el=>{const key=el.getAttribute('data-lang-key');const value=translations[lang]?.[key];if(value!==undefined&&value!==null){if(el.tagName==='INPUT'||el.tagName==='TEXTAREA'){el.setAttribute('placeholder',value)}else if(el.tagName==='IMG'){el.setAttribute('alt',value)}else if(el.hasAttribute('aria-label')){el.setAttribute('aria-label',value);const bold=el.querySelector('b');if(bold){bold.textContent=value}else{el.textContent=value}}else{const bold=el.querySelector('b');if(bold){bold.textContent=value}else{el.textContent=value}}}});console.log(`✅ Traduzioni applicate in ${lang}`)}
