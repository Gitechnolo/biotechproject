function QRedshift(){if(document.body.classList.contains('qredshift-active'))return;const hour=new Date().getHours();let filter;if(hour>=7&&hour<19){filter='sepia(0.2) hue-rotate(0deg) brightness(1)'}else{filter='sepia(0.6) hue-rotate(-30deg) brightness(1)'}
document.body.classList.add('qredshift-active');document.body.style.filter=filter;document.body.style.transition='filter 0.5s';const menuContainer=document.getElementById('tech-main-menu');if(menuContainer){const menuItem=document.createElement('div');menuItem.className='tech-menu-item';menuItem.setAttribute('data-menu','qredshift');const button=document.createElement('button');button.className='tech-nav-btn';button.type='button';button.setAttribute('aria-haspopup','false');button.setAttribute('aria-expanded','false');button.setAttribute('aria-pressed','true');button.setAttribute('aria-label','Modalità comfort visivo attiva: Notte');button.innerHTML='<b>🌙 Comfort</b>';menuItem.appendChild(button);menuContainer.appendChild(menuItem);button.addEventListener('click',function(){document.body.classList.remove('qredshift-active');document.body.style.filter='';const particles=document.getElementById('particles-canvas');if(particles)particles.style.display='none';const dna=document.querySelector('.dna-container-8');if(dna)dna.style.display='none';button.setAttribute('aria-pressed','false');button.setAttribute('aria-label','Modalità comfort visivo disattivata');button.innerHTML='<b>☀️ Comfort</b>'})}else{const icon=document.createElement('div');icon.className='qredshift-icon';icon.setAttribute('role','button');icon.setAttribute('tabindex','0');icon.setAttribute('aria-label','Disattiva comfort visivo');icon.title='Comfort visivo QRedshift attivo - clicca per disattivare';icon.style.cssText=`
      position: fixed;
      top: 10px;
      right: 10px;
      width: 36px;
