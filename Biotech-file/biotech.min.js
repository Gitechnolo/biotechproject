...
document.addEventListener("DOMContentLoaded",()=>{const observer=new MutationObserver((mutations)=>{mutations.forEach((mutation)=>{mutation.addedNodes.forEach((node)=>{if(node.nodeType===1&&node.classList?.contains('qredshift-icon')){node.onclick=null;node.addEventListener('click',function(){document.body.classList.remove('qredshift-active');document.body.style.filter='';const particlesCanvas=document.getElementById('particles-canvas');if(particlesCanvas)particlesCanvas.style.display='none';const dnaContainer=document.querySelector('.dna-container-8');if(dnaContainer)dnaContainer.style.display='none';node.remove()})}})})});observer.observe(document.body,{childList:!0,subtree:!0})})