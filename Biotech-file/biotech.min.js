function QRedshift(){if(document.body.classList.contains('qredshift-active'))return;const hour=new Date().getHours();let temp,gamma;if(hour>=7&&hour<19){temp=5900;gamma=1.0}else{temp=3500;gamma=1.0}
let filter;if(temp===5900){filter='sepia(0.2) hue-rotate(0deg) brightness(1)'}else{filter='sepia(0.5) hue-rotate(-30deg) brightness(1)'}
document.body.classList.add('qredshift-active');document.body.style.filter=filter;document.body.style.transition='filter 0.5s';const icon=document.createElement('div');icon.className='qredshift-icon';icon.title='Comfort visivo QRedshift attivo';icon.innerHTML='<span>🌙</span>';document.body.appendChild(icon);icon.addEventListener('click',()=>{document.body.classList.remove('qredshift-active');document.body.style.filter='';icon.remove()})}
...
function openSupportPopup(){openPopup('https://gitechnolo.github.io/biotechproject/O.S_support.html','O.S. Support Chat GPT',760,370)}
...
function applyTheme(index){const theme=themes[index];document.documentElement.style.setProperty('--color-accent-rgb',theme.rgb);document.documentElement.style.setProperty('--color-accent-h',theme.h);document.documentElement.style.setProperty('--color-accent-s',theme.s);document.documentElement.style.setProperty('--color-accent-l',theme.l);document.documentElement.style.setProperty('--color-glow',`hsl(${theme.h}, 100%, 70%)`);themeBtn.textContent=`← 🎨 T e m a (${theme.name}) →`;updateAriaLabel(theme.name)}
