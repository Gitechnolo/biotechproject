function QRedshift(){const menuContainer=document.getElementById('tech-main-menu');const storageKey='qredshift_disabled';const isDisabledFromStorage=localStorage.getItem(storageKey)==='true';if(isDisabledFromStorage){const style=document.createElement('style');style.id='qredshift-fouwc-fix';style.innerHTML=`
      #particles-canvas, 
      .dna-container-8 {
        visibility: hidden; 
...
if(!menuContainer)return;const particles=document.getElementById('particles-canvas');const dna=document.querySelector('.dna-container-8');let isActive=!isDisabledFromStorage;if(isActive){const fouwcStyle=document.getElementById('qredshift-fouwc-fix');if(fouwcStyle)fouwcStyle.remove();}else{}
const toggleQRedshift=function(){isActive=!isActive;if(isActive){const fouwcStyle=document.getElementById('qredshift-fouwc-fix');if(fouwcStyle)fouwcStyle.remove();}else{}};button.addEventListener('click',toggleQRedshift)}
