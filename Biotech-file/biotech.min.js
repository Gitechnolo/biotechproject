...
menubar=no,
location=no
`;const popup=window.open(url,title,options);if(!popup){alert("Il popup è stato bloccato. Per favore, abilita i popup per questo sito.")}else{popup.focus()}}
...
function applyTranslations(translations,lang){document.querySelectorAll('[data-lang-key]').forEach(el=>{const key=el.getAttribute('data-lang-key');const value=translations[lang]?.[key];if(value!==undefined&&value!==null){if(el.tagName==='INPUT'||el.tagName==='TEXTAREA'){el.setAttribute('placeholder',value)}else if(el.tagName==='IMG'){el.setAttribute('alt',value)}else if(el.hasAttribute('aria-label')){el.setAttribute('aria-label',value)}else{el.textContent=value}}})}
function getSavedLanguage(){return localStorage.getItem('preferred-language')||(navigator.language.startsWith('en')?'en':'it')}
function setLanguage(lang){const pageName=getPageName();if(!translatablePages.includes(pageName)){loadTranslation('lang/common.json').then(common=>{if(!common)return;const translations={it:common.it||{},en:common.en||{}};applyTranslations(translations,lang);updateLanguageButton(lang);document.documentElement.lang=lang;currentLang=lang;localStorage.setItem('preferred-language',lang)});return}
...
applyTranslations(translations,lang);updateLanguageButton(lang);document.documentElement.lang=lang;currentLang=lang;localStorage.setItem('preferred-language',lang)})}
