...
function setLanguage(lang){const pageName=getPageName();if(!translatablePages.includes(pageName)){loadTranslation('lang/common.json').then(common=>{if(!common)return;const translations={it:common.it||{},en:common.en||{}};setTimeout(()=>{applyTranslations(translations,lang);updateLanguageButton(lang);document.documentElement.lang=lang;currentLang=lang;localStorage.setItem('preferred-language',lang);const activeFilter=document.querySelector('.filter-btn.active')?.dataset.filter;if(activeFilter)filterSelection(activeFilter);},100)});return}
...
setTimeout(()=>{applyTranslations(translations,lang);updateLanguageButton(lang);document.documentElement.lang=lang;currentLang=lang;localStorage.setItem('preferred-language',lang);const activeFilter=document.querySelector('.filter-btn.active')?.dataset.filter;if(activeFilter)filterSelection(activeFilter);},100)})}
