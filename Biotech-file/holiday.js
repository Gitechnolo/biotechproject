 ...
 */
/**
 * BIOTECH CORE SYSTEM - Version 4.5 ZERO-FAULT
 * SRE Pillars: Data Integrity, State Consistency, Resource Management
 */
(() => {
    "use strict";

    // --- 1. CONFIGURAZIONE CENTRALIZZATA (Immutable) ---
    const CONFIG = Object.freeze({
        DOM_IDS: { banner: "Banner" },
        URLS: {
            banners: [
        ...
        TIMINGS: { bannerRotation: 3500, popupTTL: 6000, syncInterval: 1000 },
        LOGS: {
    ...
    // --- 2. STATO CENTRALIZZATO (Resource Management) ---
    const STATE = {
        intervals: { banner: null, popupSync: null, ttl: null },
    ...
    const HolidayCalcs = {
        ...
        relative: (calcFn, offset) => (y) => {
            const base = calcFn(y);
            const date = new Date(y, base.month, base.day + offset);
    ...
    const getProcessedHolidays = () => {
        ...
        // Data Integrity: Cache deterministica
        if (STATE.cache.key === cacheKey) return STATE.cache.data;

        const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
        const currentYear = today.getFullYear();

        const processed = HOLIDAY_SCHEMA.map(h => {
            let res = h.calc(currentYear);
            let date = new Date(currentYear, res.month, res.day);
            if (date < today) {
                res = h.calc(currentYear + 1);
                date = new Date(currentYear + 1, res.month, res.day);
            }
            return { ...h, date, diff: Math.ceil((date - today) / 86400000) };
        }).sort((a, b) => a.diff - b.diff);

        STATE.cache = { key: cacheKey, data: processed };
        return processed;
    };

    // --- 4. UI ENGINE (Null-Safety) ---
    const showHolidayPopup = (holiday) => {
        ...
        STATE.ui.activePopup = popup;

        const closePopup = () => {
            cleanupResources('ttl');
            cleanupResources('popupSync');
            popup.classList.remove('show');
            setTimeout(() => popup.remove(), 600);
        };

        STATE.intervals.ttl = setTimeout(closePopup, CONFIG.TIMINGS.popupTTL);

        if (holiday.isToday) {
            STATE.intervals.popupSync = setInterval(() => {
                const now = new Date();
                ...
                if (textEl) {
                    textEl.innerHTML = `${holiday.icon} ${holiday.wish} <br><small style="opacity:0.8; font-size:0.8em;">Ends in: ${timeStr}</small>`;
                }
                console.log(`%c ⏱️ SRE LIVE SYNC: ${holiday.name} active. Time to TTL: ${timeStr}`, "color: #888; font-size: 9px;");
            }, CONFIG.TIMINGS.syncInterval);
        }
        ...
        popup.querySelector('.popup-export')?.addEventListener('click', (e) => {
            ...
            console.log("%c 🟢 SRE Action: Export triggered. Manual override active (TTL Suspended).", "color: #00c853;");
        });
    ...
    // --- 5. EXPORT & DATA INTEGRITY ---
    const exportHolidayData = (format = 'json') => {
        try {
            ...
            if (format === 'csv') {
                ...
                const rows = data.map(item => [item.icon, item.name, item.date.toISOString().split('T')[0], item.diff, "SRE_VERIFIED"]);
                // UTF-8 integrity: Aggiunta del BOM per compatibilità universale
                const csvContent = "\ufeff" + [headers, ...rows].map(e => e.join(",")).join("\n");
                downloadFile(csvContent, `biotech_export_${new Date().getFullYear()}.csv`, 'text/csv;charset=utf-8');
            } else {
                const payload = { system: "Biotech-Core", exportedAt: exportTime, holidays: data.map(h => ({ name: h.name, icon: h.icon, date: h.date.toISOString().split('T')[0], daysLeft: h.diff })) };
                downloadFile(JSON.stringify(payload, null, 2), 'biotech_holidays.json', 'application/json;charset=utf-8');
            }
        } catch (err) {
            console.error("%c 🚨 Export Failed:", "color: red;", err);
        }
    };
    ...
    const bootstrap = async () => {
        console.log("%c 🛠️ SRE Bootstrap %c Initializing Biotech Core System...", CONFIG.LOGS.display + CONFIG.LOGS.base, "color: #888; font-style: italic; margin-left: 5px;");

        try {
            const bannerImg = document.getElementById(CONFIG.DOM_IDS.banner);
            ...
            if (!(conn.saveData || /2g|slow-2g/.test(conn.effectiveType))) {
                CONFIG.URLS.banners.forEach(src => { const img = new Image(); img.src = src; });
            } else {
                console.warn("%c 📡 SRE ALERT: Low Bandwidth detected.", "color: #ff9800;");
            }

            let bnrCntr = 0;
            STATE.intervals.banner = setInterval(() => {
                bannerImg.classList.add("banner-fade-out");
                setTimeout(() => {
                    bnrCntr = (bnrCntr + 1) % CONFIG.URLS.banners.length;
                    bannerImg.src = CONFIG.URLS.banners[bnrCntr];
                    bannerImg.classList.remove("banner-fade-out");
                }, 500);
            }, CONFIG.TIMINGS.bannerRotation);

            await new Promise(res => window.addEventListener("load", res, { once: true }));
            
            const nextHoliday = getNextHoliday();
            showHolidayPopup(nextHoliday);
            triggerHumanSync();

        } catch (err) {
            console.error("%c 🚨 SRE Pipeline Error:", "color: #ff5252; font-weight: bold;", err.message);
        }
    };
    ...
    function getNextHoliday() {
        const now = new Date();
        const t = {
            ts: now.toTimeString().split(' ')[0],
            isDay: now.getHours() >= 6 && now.getHours() < 18
        };
        const icon = t.isDay ? "☀️" : "🌙";

        console.log(
            `%c ${icon} ${t.isDay ? 'Daylight' : 'Nightly'} %c | ${t.ts} %c Syncing dynamic cycles for ${now.getFullYear()}`,
            CONFIG.LOGS.astro + CONFIG.LOGS.base, "color: #888; font-family: monospace;", "color: #555; font-style: italic;"
        );

        const upcoming = getProcessedHolidays();

        console.groupCollapsed('%c📅 Upcoming Holidays Table', CONFIG.LOGS.display + CONFIG.LOGS.base);
        ...
        const style = CONFIG.LOGS[next.style] || CONFIG.LOGS.display;

        console.log(
            `%c ${next.icon} BiotechHoliday %c ${next.name}: ${isToday ? 'ACTIVE' : next.diff + 'd left'}`,
            style + CONFIG.LOGS.base, "color: #bcbcbc; margin-left: 5px;"
        );
    ...
    // Expose Global API (SRE Entry Point)
