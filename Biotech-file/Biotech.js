...
// 1. LOGICA PARTICELLE 
// =======================================
// 🔥 OTTIMIZZAZIONE COMPLETA PARTICELLE
(function () {
  'use strict';

  // Rileva dispositivi con bassa potenza
  const isLowEndDevice = () => {
    const hwConcurrency = navigator.hardwareConcurrency || 8;
    const deviceMemory = navigator.deviceMemory || 4;
    return hwConcurrency < 4 || deviceMemory < 4;
  };
  ...
  function initParticles(canvasId, options = {}) {
    ...
    const shadowBlur = isMobile ? 6 : 10;

    // Adatta il conteggio in base al dispositivo
    const baseCount = isDay ? (options.count || 50) : (options.count || 30);
    const count = isLowEndDevice() ? Math.floor(baseCount * 0.6) : baseCount;
