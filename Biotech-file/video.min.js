...
const imageIds=["myImg","myImg2","myImg3","myImg4"];imageIds.forEach(function(imgId){const img=document.getElementById(imgId);if(img){img.addEventListener("click",function(){openModal(this)})}});closeBtn.onclick=closeModal;closeBtn.addEventListener("keydown",function(event){if(event.key==='Enter'||event.key===' '){event.preventDefault();closeModal()}});modal.onclick=function(e){if(e.target===modal){closeModal()}};document.addEventListener("keydown",function(event){if(event.key==='Escape'&&modal.style.display==="block"){event.preventDefault();closeModal()}})})