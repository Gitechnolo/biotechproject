...
document.addEventListener("DOMContentLoaded",function(){const modal=document.getElementById("myModal");const modalImg=document.getElementById("img01");const captionText=document.getElementById("caption");const closeBtn=modal?modal.querySelector(".close"):document.querySelector("#myModal .close");let lastFocusedElement=null;if(!modal||!modalImg||!captionText||!closeBtn){console.warn("Elementi critici del Modal non trovati. Controllo: #myModal, #img01, #caption, .close.");return}
function openModal(imgElement){lastFocusedElement=imgElement;modal.style.display="block";modalImg.src=imgElement.src;captionText.textContent=imgElement.alt||"";closeBtn.focus()}
function closeModal(){modal.style.display="none";modalImg.src="";captionText.textContent="";if(lastFocusedElement){lastFocusedElement.focus();lastFocusedElement=null}}
const imageIds=["myImg","myImg2","myImg3","myImg4","myImg5","myImg6"];imageIds.forEach(function(imgId){const img=document.getElementById(imgId);if(img){img.addEventListener("click",function(){openModal(this)});img.addEventListener("keyup",function(event){if(event.key==='Enter'||event.key===' '){event.preventDefault();openModal(this)}})}});closeBtn.onclick=closeModal;closeBtn.addEventListener("keyup",function(event){if(event.key==='Enter'||event.key===' '){event.preventDefault();closeModal()}});modal.onclick=function(e){if(e.target===modal){closeModal()}};document.addEventListener("keydown",function(event){if(event.key==='Escape'&&modal.style.display==="block"){event.preventDefault();closeModal()}})})